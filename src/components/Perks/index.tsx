...
const calcRowSize = (windowWidth: number) => {
  if (windowWidth < (4 * MOBILE_PERK_WIDTH + 3 * MOBILE_PERK_GAP + MOBILE_EXTRA_OFFSET + MOBILE_PADDING * 2 + FREE_WIDTH)) return 3;
  if (windowWidth < (5 * MOBILE_PERK_WIDTH + 4 * MOBILE_PERK_GAP + MOBILE_EXTRA_OFFSET + MOBILE_PADDING * 2 + FREE_WIDTH)) return 4;
  if (windowWidth < TABLET_WINDOW_WIDTH) return 5;
  if (windowWidth < (4 * TABLET_PERK_WIDTH + 3 * TABLET_PERK_GAP + TABLET_EXTRA_OFFSET + TABLET_PADDING * 2 + FREE_WIDTH)) return 3;
  if (windowWidth < (5 * TABLET_PERK_WIDTH + 4 * TABLET_PERK_GAP + TABLET_EXTRA_OFFSET + TABLET_PADDING * 2 + FREE_WIDTH)) return 4;
  return 5;
}
...
function EmptySlot({ className = '' }: EmptySlotProps): React.JSX.Element {
  return (
    <div className={`perks__empty-slot ${className}`} />
  )
}
...
export default function Perks({ className = '', perks }: PerksProps): React.JSX.Element {
