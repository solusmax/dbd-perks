...
function SideImg({ className = '', side }: SideImgProps): React.JSX.Element {
  ...
  return (
    <img
      className={`perk__side-img ${className}`}
    ...
    <div className={`perk ${className}`}>
      ...
      >
        <img
          className="perk__img"
          src={`./img/perks/${side}/${
            !(isLegacyMode && legacy !== null) ? icon : 'legacy/' + legacy.icon
          }`}
          alt={t(!isLegacyMode ? `${id}.name` : `${id}.legacy.name`, {
            ns: 'perks',
          })}
        />
      </button>
      ...
      <Tooltip
        ...
        middlewares={[
          ...
          shift({
            padding:
              windowWidth < TABLET_WINDOW_WIDTH
                ? MOBILE_PADDING
      ...
      >
        <div className="perk__header">
          <div className="perk__name">
            <a
              className="perk__wiki-link"
              href={!(isLegacyMode && legacy !== null) ? wiki : legacy.wiki}
              target="_blank"
              rel="noreferrer"
            >
              {t(!isLegacyMode ? `${id}.name` : `${id}.legacy.name`, {
                ns: 'perks',
              })}
            </a>
          </div>
          <div className="perk__subtitle">
            {!isCurrentLanguageEnglish()
              ...
              : ''}
            {t(
              !(isLegacyMode && legacy !== null) ? character : legacy.character,
              { ns: 'characters' },
            )}
          </div>
          <div className="perk__side">
            {legacy !== null ? (
              <button
                className="perk__legacy-switcher"
                type="button"
                onClick={handleLegacySwitcherClick}
              >
                <SideImg side={side} />
              </button>
            ) : (
        ...
        <div
          className="perk__description"
          dangerouslySetInnerHTML={{
            __html: t(`${id}.description`, { ns: 'perks' }),
          }}
        />
      </Tooltip>
    </div>
